...
> **Status: SEEDED (v0).** `collect-apmphp.sh` is a working Tier-0 collector
...
tracer modules — then exercised against a live install (Apache 2.4 prefork +
mod_php 8.2 container, agent 2.14.2 installed with the vendor `install.sh`).
...
| 3 | PHP runtimes and SAPIs | every php / php-fpm / php-cgi binary found (PATH, usual install paths, running processes): version, **SAPI**, **PHP API**, **Thread Safety**, build strings, `extension_dir`, the ini paths it parses, opcache/JIT settings, the **`whatap.*` directives as that binary actually resolves them (local => master)**, the loaded module list, plus **other APM/profiler extensions** present (newrelic, ddtrace, elastic, opentelemetry, tideways, blackfire, xdebug, …) |
| 4 | Web server / application server layer | Apache binary + `-V` (**MPM prefork/worker/event** — decides whether a `_zts` module is required) and its php/mpm modules; php-fpm version, config and pool files; nginx; every web/php process with cmdline, exe and uid; **persistent-worker runtimes** (Swoole/Laravel Octane, RoadRunner, FrankenPHP, Workerman, php-pm) whose request cycle is not the per-request PHP model the tracer hooks |
...
| 6 | Tracer binding | every `extension_dir` seen (from `php -i` and from `WHATAP_PHP_EXT_HOME` in the service files); `whatap.so` there with its **symlink target decoded** (thread-safe build yes/no, PHP API); every `whatap.ini` found across the RHEL / Debian per-SAPI / Alpine / source-build / agent-home ini locations; `php.ini` files carrying whatap lines (the installer's fallback when PHP reports no scan dir); which ini trees exist and whether each holds a whatap entry; **live load status from `/proc/<pid>/maps`** |
...
capped process and binary detail: 6 PHP binaries, 20 processes), per-probe
