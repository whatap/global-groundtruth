...
| Tier | Flags | Behavior |
|---|---|---|
| 0 (default) | `--file` / `--stdout` | read-only API GETs, bounded log tails (env `LOG_TAIL_LINES`, default 200; `--tail` is refused since 0.11.0), exec into at most 2 agent pods (one `kubectl exec` per pod runs all its probes, each through its own `sh -c` and exit status; a probe that hangs says timed out and the probes after it are run in an exec of their own); the jsonpath reads of one object or list share one GET, split on marker lines, and a failed shared GET is the reason of every read it carried; every call double-bounded (kubectl `--request-timeout=15s` + the shared `_bounded` cap, 20 s, inside the 300 s run deadline); helm calls bounded the same way |
